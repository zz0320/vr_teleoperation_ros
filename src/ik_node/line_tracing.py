...
class TraceALine:
    def __init__(self):
        ...
        self.target_pose_stamped_pub = rospy.Publisher('/a2_ee_target', PoseStamped, queue_size=1)
    ...
    def generate_trajectory(self):
        ...
        for k in range(self.robot.num_chain):
            x.append(self.starting_ee_poses[k].position.x)
    ...
    def timer_callback(self, event):
        ...
        if self.use_topic_not_service:
            ...
            for i in range(self.robot.num_chain):
                ...
                target_pose_stamped.pose.position.x = self.trajectory[self.trajectory_index][i].position.x
                target_pose_stamped.pose.position.y = self.trajectory[self.trajectory_index][i].position.y
                target_pose_stamped.pose.position.z = self.trajectory[self.trajectory_index][i].position.z

                target_pose_stamped.pose.orientation.x = self.trajectory[self.trajectory_index][i].orientation.x
                target_pose_stamped.pose.orientation.y = self.trajectory[self.trajectory_index][i].orientation.y
                target_pose_stamped.pose.orientation.z = self.trajectory[self.trajectory_index][i].orientation.z
                target_pose_stamped.pose.orientation.w = self.trajectory[self.trajectory_index][i].orientation.w
